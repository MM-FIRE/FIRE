        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            for i, (role, message) in enumerate(messages):
                if message:
                    ...
                    ret += role + ":"
        elif self.sep_style == SeparatorStyle.FEEDBACK:
            seps = [self.sep, self.sep2]
            ret = self.system + seps[0]
            for i, (role, message) in enumerate(messages):
                if message:
                    if type(message) is tuple:
                        message, _, _ = message
                    sep_index = 0 if role == "USER" else 1
                    ret += role + ": " + message + seps[sep_index]
                else:
                    ret += role + ":"
            ...
            #         role_value = "user" if "user" in role else "assistant"
            #         chat_template_messages.append({"role": role_value, "content": message})
                    
            # # FIXME tokenizer looks like has a bug, add_generation_prompt failed
            # prompt = self.tokenizer.apply_chat_template(chat_template_messages, tokenize=False, add_generation_prompt=False)
            # idx = prompt.rfind("<|start_header_id|>assistant<|end_header_id|>")
            # prompt = prompt[:idx]
            # return prompt
            chat_template_messages = [{"role": "system", "content": self.system}]
            for role, message in messages:
...
conv_llava_v1_student_feedback = Conversation(
    system="A chat between a curious human and an artificial intelligence assistant. "
           "The assistant gives helpful, detailed, and polite answers to the human's questions.",
    roles=("USER", "ASSISTANT"),
    version="v1_feedback",
    messages=(),
    offset=0,
    sep_style=SeparatorStyle.FEEDBACK,
    sep=" ",
    sep2="</s>",
)

conv_llava_v1_feedback_teacher = Conversation(
    system="""A chat between a curious human and an artificial intelligence assistant. 
...
# conv_llava_llama_3_teacher = Conversation(
#     system="""A chat between a curious human and an artificial intelligence assistant. 
# The human gives a question about an image, his initial thought and answer to the question, and the groundtruth answer to this question. The assistant compares the human's answer with the groundtruth answer and provides helpful, detailed, and polite feedback to help the human improve his answer. The human revises his thought and answer based on the assistant's feedback and the assistant continues to provide further feedback on the revised answer.
...
# Original lmms-lab LLAVA-NEXT
conv_llava_llama_3_original = Conversation(
    system="You are a helpful language and vision assistant. " "You are able to understand the visual content that the user provides, " "and assist the user with a variety of tasks using natural language.",
    roles=("user", "assistant"),
    version="llama_v3_original",
    messages=[],
    offset=0,
    sep_style=SeparatorStyle.LLAMA_3,
    tokenizer_id="meta-llama/Meta-Llama-3-8B-Instruct",
...
conv_templates = {
    ...
    "llava_v1_student_feedback": conv_llava_v1_student_feedback,
    "llava_v1_teacher_feedback": conv_llava_v1_feedback_teacher,
    "mpt": conv_mpt,
    "llama_v3": conv_llava_llama_3,
    "llama_v3_student": conv_llava_llama_3_student,
    "llama_v3_teacher": conv_llava_llama_3_teacher,
    "llama_v3_original": conv_llava_llama_3_original,
}
...
if __name__ == "__main__":
    ...
    conv.append_message(conv.roles[1], "Thought:XXX\nAnswer:XXX")
    print(conv.get_prompt())
    
    conv = conv_llava_v1_feedback_teacher.copy()
    conv.append_message(conv.roles[0], "<image>\n What is in the image?\nGroudtruth:XXX\nThought:xxx\nAnswer:XXXX")
    conv.append_message(conv.roles[1], "Score:XXX\nFeedback:XXX")
    print(conv.get_prompt())
